...
import PullToRefresh from '@/componentes/PullToRefresh'
...
// CATEGORIAS_CATALOGO coordinado con CATEGORIAS_POR_TIPO del admin
// Los valores DEBEN coincidir exactamente con lo que el admin guarda al crear un producto
...
export default function PaginaCatalogo() {
  ...
  // Función para cargar productos (reutilizable para pull-to-refresh)
  ...
  const productosFiltrados = productos
    .filter(p => {
      ...
      // Filtro por categoría (valores coinciden exactamente con el admin)
      let coincideCategoria = true
      if (categoriaActiva !== 'todos') {
        const catProd = (p.categoria || '').toLowerCase()
        coincideCategoria = catProd === categoriaActiva.toLowerCase()
      }
  ...
  return (
    <>
      ...
      <PullToRefresh onRefresh={cargarProductos}>
        <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800">
          ...
          {/* GALERÍA DE FOTOS - SWIPE + ZOOM */}
          {galeriaAbierta && productoSeleccionado && modalListo && (
            <GaleriaProducto
              imagenes={obtenerImagenes(productoSeleccionado)}
              indiceInicial={imagenGaleriaActiva}
