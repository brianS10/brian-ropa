...
import AnimacionCarrito from '@/componentes/AnimacionCarrito'
...
export default function PaginaTienda() {
  ...
  const handleAgregar = (variante, producto, e) => {
    agregarProducto({
    ...
    if (e && carritoRef.current && typeof window !== 'undefined' && window.dispararAnimacionCarrito) {
      const rect = e.currentTarget.getBoundingClientRect()
      const carritoRect = carritoRef.current.getBoundingClientRect()
      window.dispararAnimacionCarrito({
        fromX: rect.left + rect.width / 2 - 28,
        fromY: rect.top + rect.height / 2 - 28,
        toX: carritoRect.left + carritoRect.width / 2 - 28,
        toY: carritoRect.top + carritoRect.height / 2 - 28,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800">
                          ...
                          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                            {imagenesProducto.map((_, idx) => (
                              <button
                                ...
                                className={`w-2 h-2 rounded-full transition-all ${idx === imagenActiva
                                    ? 'bg-white w-4'
                                    : 'bg-white/50'
                                  }`}
                              />
                ...
                <div className="flex flex-wrap gap-2">
                  {productoSeleccionado.variantes_producto?.map(variante => {
                    ...
                    return (
                      <button
                        key={variante.id}
                        onClick={(e) => !sinStock && handleAgregar(variante, productoSeleccionado, e)}
                        disabled={sinStock}
                        className={cn(
      ...
      <a
        ...
        className={`fixed z-50 w-14 h-14 bg-green-500 hover:bg-green-600 text-white rounded-full shadow-lg shadow-green-500/30 flex items-center justify-center transition-all hover:scale-110 ${cantidadCarrito > 0 && !productoSeleccionado ? 'bottom-24' : 'bottom-6'
          } right-6`}
      >
